...
function jitter(baseMs) {
  return baseMs + Math.floor(Math.random() * 120);
}
...
async function rpc(method, params) {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    const endpoint = RPC_URLS[attempt % RPC_URLS.length];
    try {
      ...
      if (r.status === 429) {
        const retryAfterSec = Number(r.headers.get('retry-after') || 0);
        const backoff = retryAfterSec > 0 ? retryAfterSec * 1000 : jitter(400 * (attempt + 1));
        await sleep(backoff);
        continue;
      }

      if (!r.ok) throw new Error(`RPC HTTP ${r.status} (${endpoint})`);

      const j = await r.json();
      if (j.error) {
        const code = Number(j.error.code);
        const isRateLimited = code === 429 || String(j.error.message || '').toLowerCase().includes('rate limit');
        if (isRateLimited) {
      ...
      const transient = msg.includes('429') || msg.includes('fetch failed') || msg.includes('ECONNRESET') || msg.includes('ETIMEDOUT');
      if (transient && attempt < maxAttempts - 1) {
        await sleep(jitter(350 * (attempt + 1)));
        continue;
      }
    }
  }
...
async function loadHolders(mint) {
  // Fastest path first: largest accounts are enough for top holder/whale analytics UI
  try {
    const largestOwners = await loadLargestAccountsOwners(mint);
    if (largestOwners.length) return largestOwners;
  } catch {
    // continue
  }

  // Next: direct mint query
  try {
...
async function analyzeMint(mint) {
  ...
  const whaleSource = topOwners.filter((h) => ((h.amountUi / totalSupply) * marketCapNow) >= 10000).slice(0, 12);

  async function enrich(ownerObj, withTsLookup = true) {
    const tokenAccount = ownerObj.tokenAccounts[0];
    const ts = withTsLookup ? await firstSeenTs(tokenAccount) : null;
    const entryIndex = ts ? nearestIndexByTs(series, ts) : ownerHashToIndex(ownerObj.owner, series.length);
  ...
  const [top5, whales] = await Promise.all([
    Promise.all(top5Source.map((o) => enrich(o, true))),
    Promise.all(whaleSource.map((o) => enrich(o, true))),
  ]);

  // Media all-holders rapida su campione senza ulteriori RPC heavy
  const sampleFast = topOwners.slice(0, 35).map((o) => {
    const idx = ownerHashToIndex(o.owner, series.length);
    const entryMcap = Number(series[idx]?.value || marketCapNow || 0);
    return entryMcap > 0 ? ((marketCapNow / entryMcap) - 1) * 100 : 0;
  });

  const avgTop5PnlPct = top5.length ? top5.reduce((s, x) => s + x.pnlPct, 0) / top5.length : 0;
  const avgAllHoldersPnlPct = sampleFast.length ? sampleFast.reduce((s, x) => s + x, 0) / sampleFast.length : 0;
  ...
  return {
    ...
    chartNote: `Dati reali on-chain. Holder: ${holdersRaw.length}. Top5 e whale da RPC Solana; mcap da Dexscreener; serie storica da GeckoTerminal. Media 'all holders' stimata su campione rapido top wallet.`,
  };
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (u.pathname === '/api/health') {
    return json(res, 200, { status: 'ok', baseDir: BASE_DIR, rpcConfigured: RPC_URLS.length > 0, rpcEndpoints: RPC_URLS.length });
  }

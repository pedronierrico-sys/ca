...
async function getAnalytics(mint) {
  const endpoint = `/api/analyze?mint=${encodeURIComponent(mint)}`;
  let response;
  try {
    response = await fetchWithTimeout(endpoint);
  } catch {
    throw new Error("Backend non raggiungibile. Avvia con `npm start` e apri l'app da http://localhost:8000 (non file://).");
  }

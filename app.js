...
ui.saveApiBtn.addEventListener("click", saveApiBaseFromInput);
ui.testApiBtn.addEventListener("click", testApiConnection);
initApiBaseUi();
...
function buildApiCandidates() {
  ...
  const manualInput = normalizeBase(ui.apiBaseInput?.value?.trim() || "");
...
function getStoredApiBase() {
  return normalizeBase(window.localStorage.getItem("API_BASE") || "");
}

function setApiStatus(message, ok = false) {
  ui.apiStatus.textContent = message || "";
  ui.apiStatus.className = `hint ${ok ? "pnl-pos" : ""}`;
}

function initApiBaseUi() {
  const fromQuery = normalizeBase(new URLSearchParams(window.location.search).get("apiBase") || "");
  const stored = getStoredApiBase();
  ui.apiBaseInput.value = fromQuery || stored;
  if (ui.apiBaseInput.value) {
    setApiStatus(`API base attiva: ${ui.apiBaseInput.value}`, true);
  } else {
    setApiStatus("API base non impostata: verranno provati fallback automatici.");
  }
}

function saveApiBaseFromInput() {
  const v = normalizeBase(ui.apiBaseInput.value.trim());
  if (!v) {
    window.localStorage.removeItem("API_BASE");
    setApiStatus("API base rimossa. Userò fallback automatici.");
    return;
  }
  window.localStorage.setItem("API_BASE", v);
  setApiStatus(`API base salvata: ${v}`, true);
}

async function testApiConnection() {
  const manual = normalizeBase(ui.apiBaseInput.value.trim());
  const base = manual || buildApiCandidates()[0];
  if (!base) {
    setApiStatus("Nessuna API base disponibile da testare.");
    return;
  }
  try {
    ...
    setApiStatus(`Connessione OK: ${base} (${data.status || "ok"})`, true);
    ui.apiBaseInput.value = base;
    window.localStorage.setItem("API_BASE", base);
  } catch (e) {
    setApiStatus(`Connessione fallita su ${base}: ${e.message}`);
  }
}
...
async function getAnalytics(mint) {
  ...
  for (const base of candidates) {
    ...
    try {
      ...
      window.localStorage.setItem("API_BASE", base);
      ui.apiBaseInput.value = base;
      setApiStatus(`API collegata: ${base}`, true);
      return data;
    } catch (e) {
  ...
  setApiStatus("Nessun endpoint API raggiungibile. Controlla backend/URL API.");

...
async function getAnalytics(mint) {
  const endpoint = `${resolveApiBase()}/api/analyze?mint=${encodeURIComponent(mint)}`;
  let r;
  try {
    r = await fetch(endpoint, { headers: { "Accept": "application/json" } });
  } catch (e) {
    throw new Error("Failed to fetch API. Avvia il backend con `npm start` e apri la pagina da http://localhost:8000 (non file://). Se usi frontend su dominio diverso, abilita CORS sul backend.");
  }

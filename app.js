...
async function getAnalytics(mint) {
  ...
  for (const endpoint of candidates) {
    try {
      ...
      errors.push(`${endpoint}: ${e?.message || 'fetch failed'}`);
    }
  }

  throw new Error(`Backend non raggiungibile. Endpoint provati: ${errors.join(' | ')}. Avvia con npm start e apri l'app da http://localhost:8000 (non file://).`);
}
